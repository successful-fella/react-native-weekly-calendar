...
const WeeklyCalendar = (props) => {
	...
	const createEventMap = (events) => {
		...
		for (let i = 0; i < events.length; i++) {
			let eventDate = moment(events[i].start)
				.format("YYYY-MM-DD")
				.toString()
			...
			if (events !== undefined) {
				if (props.renderEvent !== undefined) {
					...
					eventViews = events.map((event, j) => {
						...
						return (
							<View key={i + "-" + j}>
                                <TouchableOpacity onPress={onEventPress.bind(this, event_id)} >
                                    <View style={styles.event}>
                                        <View style={styles.eventDuration}>
                                            ...
                                            <View
                                                style={styles.durationDotConnector}
                                            />
                                        </View>
	...
	const pickerOnChange = (_event, pickedDate) => {
		if (Platform.OS === "android") {
			...
			if (pickedDate !== undefined) {
				...
				setTimeout(() => {
					let pickedDateMoment = moment(pickedDate).locale(
						props.locale
					)
    ...
    const onEventPress = (event_id) => {
		if (props.onEventPress !== undefined) props.onEventPress(event_id)
	}
